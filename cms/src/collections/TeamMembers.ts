...
const upsertIntoSupabase = async ({ doc, req }: { doc: any; req: any }) => {
  ...
  try {
    ...
    await logCmsSyncEvent({
      ...
      status: 'success',
      metadata: { hasBio: Boolean(bio), linkedinConnected: Boolean(linkedin_url), active },
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error while syncing team member';
    await logCmsSyncEvent({
      collection: 'teamMembers',
      action: 'upsert',
      payloadId: doc.id ?? null,
